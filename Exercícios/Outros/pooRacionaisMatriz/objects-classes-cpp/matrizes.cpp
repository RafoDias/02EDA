...
class Matrix
{
    ...
    bool IsSquare();
    bool IsSymmetric();
    bool IsEqualTo(const Matrix rhs);
    ...
    bool Matrix::IsEqualTo(Matrix rhs)
    {
        if (rhs.columns != this->columns || rhs.rows != this->rows)
            return false;

        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < columns; j++)
            {
                if (thisMatrix[i][j] != rhs.Get(i, j))
                    return false;
            }
        }
        return true;
    ...
    Matrix Matrix::operator=(const Matrix rhs)
    {
        ...
        for (int i = 0; i < columns; i++)
        {
            ...
            for (int j = 0; j < rows; j++)
            {
                ptr[i][j] = thisMatrix[i][j];
            }
        }

        Matrix *out = new Matrix(ptr, rows, columns);
        return *out;
    }
